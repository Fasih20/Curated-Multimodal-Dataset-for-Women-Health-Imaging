"""
Source: original Colab notebook, cell index [49]
Auto-extracted -- review before treating as final.
"""

"""
Colab: Stage 7 -- image-caption dataset from the already-aligned panel-text
pairs. No synthetic caption generation -- Stage 2's best_match_text is
reliable associated text, so it's used directly.
"""
...
OUT_DIR = Path("/content/pipeline_data/captioning_v1")
CAPTIONING_PATH = OUT_DIR / "captioning_dataset.parquet"


def main() -> None:
    if CAPTIONING_PATH.exists():
        print(f"[SKIP] Stage 7 already done -- {CAPTIONING_PATH}")
        return
